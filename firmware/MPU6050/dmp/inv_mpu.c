...
#define MPU6050							// Use MPU6050
#define MOTION_DRIVER_TARGET_MSP430		// MSP430 driver transplant to STM32F4
...
#define delay_ms    HAL_Delay // delay_ms
...
// ---------- following add by user ----------

// q30 格式, long 转 float 时的除数
...
unsigned char mpu_dmp_init(void)
{
	...
	mpu6050_i2c_gpio_init(); // IIC_Init(); // 初始化 IIC 总线
	...
	if (mpu_init() == 0) { // 初始化 MPU6050
		...
		if (res)
            return 5;
		res = dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP // 设置 dmp 功能
